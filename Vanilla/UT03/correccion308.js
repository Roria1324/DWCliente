...
let objetos = [];
usuarios.map((e, i) => {
  // Añado un objeto a los que ya tiene objetos.
  objetos = [...objetos, { id: i, nombre: e }];
});

// Mucho mejor.

const objetosBien = usuarios.map((valor, indice) => {
  return { id: indice, nombre: valor };
});

// Añadir un nuevo elemento (función declarada en una biblioteca).
const agregarApellido = (usuarios) => {
  return usuarios.map((usuario) => ({ ...usuario, apellidos: "No indicado" }));
};

// Añadir código postal.
const agregarCodigoPostal = (usuarios) => {
  return usuarios.map((usuario) => ({
    ...usuario,
    contacto: {
      ...usuario.contacto,
      direccion: { ...usuario.contacto.direccion, codigoPostal: "00000" },
    },
  }));
};
